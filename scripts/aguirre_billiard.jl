...
# Jacobo Aguirre and Miguel A. F. Sanjuan ́
...
function compute_exit_three_disk(di::Dict)
    @unpack ww, res = di
    br = get_billiard(ww)
    x0 = range(-0.1, 0.1, length = res) 
    θ0 = range(0,pi,length = res)
    u(x,θ) = Particle([x, 0.0001, θ])
    bsn = [ escapewall!(u(x,θ), br, 2000) for x in x0, θ in θ0] 
    grid = (x0,θ0)
...
function print_fig(w, h, cmap, ww, res)
    ...
    data, file = produce_or_load(
        ...
        prefix = "open_disks", storepatch = false, suffix = "jld2", force = true
    )
...
print_fig(600,600, nothing, 0.2, 400) 
